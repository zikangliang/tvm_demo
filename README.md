# TVM Runtime 简化架构文档

## 1. 项目概述

本项目是 TVM Runtime 的精简嵌入式实现，采用 **8 文件架构**，在保持清晰职责分离的同时，最大化代码的可移植性和可维护性。

当前实现了一个 **16 算子 / 9 层 / 8 内存槽** 的复杂模型，用于验证内存复用和 BSP 调度的正确性。

**算子库**：支持 **15 种算子**（6 种基础算子 + 9 种扩展算子），涵盖激活函数、基础运算和常量运算。

**核心特性**:
- ✅ BSP 静态分层调度
- ✅ 极限内存复用（8 槽各复用 1-3 次）
- ✅ 保留 lib0/lib1 职责分离
- ✅ Runtime 工具单文件化
- ✅ 零动态内存分配
- ✅ 平台可移植（POSIX / RTOS）
- ✅ 零开销日志系统（编译开关控制）
- ✅ 单元测试框架（14 项测试）

---

## 2. 项目目录结构

```
tvm_demo/
├── include/
│   └── tvmgen_default.h       # TVM 生成的公共头文件
├── src/
│   ├── main.c                 # 测试程序入口
│   ├── default_lib0.c         # Workspace 分配 + 运行入口
│   ├── default_lib1.c         # Runtime 初始化逻辑
│   ├── tvmrt.h                # Runtime 统一头文件
│   ├── tvmrt.c                # Runtime 统一实现
│   ├── tvmrt_port_posix.c     # OS 适配层 (POSIX)
│   ├── model_data.c           # 模型静态描述 (16算子/9层)
│   ├── ops.c                  # 算子实现 (15种算子)
│   └── test_new_ops.c         # 单元测试
├── docs/
│   └── updates/               # 开发记录
├── Makefile
├── README.md
└── future.md                  # 未来发展规划
```

---

## 3. 各文件内容概述

### 3.1 入口与 Workspace

| 文件 | 大小 | 内容 |
|------|------|------|
| `main.c` | ~55 行 | 测试程序入口，准备输入输出并调用 `tvmgen_default_run()` |
| `default_lib0.c` | ~67 行 | 静态分配 workspace (64B) 和常量区 (68B)，提供 `tvmgen_default_run()` |
| `default_lib1.c` | ~87 行 | Runtime 初始化，算子执行表填充，调用调度引擎 |

### 3.2 Runtime 核心

| 文件 | 大小 | 内容 |
|------|------|------|
| `tvmrt.h` | ~420 行 | Runtime 完整 API：类型定义、OS 抽象、日志、引擎、语义转换 |
| `tvmrt.c` | ~389 行 | Runtime 完整实现：日志系统、语义转换、BSP 调度引擎 |
| `tvmrt_port_posix.c` | ~200 行 | POSIX 平台适配：mutex、条件变量、线程、barrier |

### 3.3 模型相关

| 文件 | 大小 | 内容 |
|------|------|------|
| `model_data.c` | ~447 行 | 静态描述表：16算子描述、9层调度表、12个张量映射、参数填充 |
| `ops.c` | ~300 行 | 15种算子实现 + 包装函数 |
| `test_new_ops.c` | ~149 行 | 单元测试（14 项测试用例） |

---

## 4. 模型说明

### 4.1 计算图 (16 算子 / 9 层)

```
输入: input = 10.0

═══════════════════════════════════════════════════════════════
Layer 1: 4路分叉 (并行)
═══════════════════════════════════════════════════════════════
  Op0:  input + 1  → M0    [fused_add]      结果: 11
  Op1:  input + 3  → M1    [fused_add_1]    结果: 13
  Op2:  input + 5  → M2    [fused_add_2]    结果: 15
  Op3:  input + 1  → M3    [fused_add]      结果: 11

  内存状态: [11, 13, 15, 11, -, -, -, -]

═══════════════════════════════════════════════════════════════
Layer 2: 两两合并 (并行)
═══════════════════════════════════════════════════════════════
  Op4:  M0 + M1 → M4    [fused_add_3]    结果: 24
  Op5:  M2 + M3 → M5    [fused_add_3]    结果: 26

  内存状态: [11, 13, 15, 11, 24, 26, -, -]

═══════════════════════════════════════════════════════════════
Layer 3: 变换 (并行, 复用 M0, M1)
═══════════════════════════════════════════════════════════════
  Op6:  M4 - 2 → M0 🔴  [fused_subtract]    结果: 22
  Op7:  M5 - 4 → M1 🔴  [fused_subtract_1]  结果: 22

  内存状态: [22, 22, 15, 11, 24, 26, -, -]

═══════════════════════════════════════════════════════════════
Layer 4: 合并到 M6
═══════════════════════════════════════════════════════════════
  Op8:  M0 + M1 → M6    [fused_add_3]    结果: 44

  内存状态: [22, 22, 15, 11, 24, 26, 44, -]

═══════════════════════════════════════════════════════════════
Layer 5: 累加链 (并行, 复用 M2, M3)
═══════════════════════════════════════════════════════════════
  Op9:  M6 + 3 → M2 🔴  [fused_add_1]    结果: 47
  Op10: M6 + 5 → M3 🔴  [fused_add_2]    结果: 49

  内存状态: [22, 22, 47, 49, 24, 26, 44, -]

═══════════════════════════════════════════════════════════════
Layer 6: 交叉合并到 M7
═══════════════════════════════════════════════════════════════
  Op11: M2 + M3 → M7    [fused_add_3]    结果: 96

  内存状态: [22, 22, 47, 49, 24, 26, 44, 96]

═══════════════════════════════════════════════════════════════
Layer 7: 最终变换 (并行, 复用 M4, M5)
═══════════════════════════════════════════════════════════════
  Op12: M7 - 2 → M4 🔴  [fused_subtract]    结果: 94
  Op13: M6 + 1 → M5 🔴  [fused_add]         结果: 45

  内存状态: [22, 22, 47, 49, 94, 45, 44, 96]

═══════════════════════════════════════════════════════════════
Layer 8: 合并 (复用 M0)
═══════════════════════════════════════════════════════════════
  Op14: M4 + M5 → M0 🔴  [fused_add_3]    结果: 139

  内存状态: [139, 22, 47, 49, 94, 45, 44, 96]

═══════════════════════════════════════════════════════════════
Layer 9: 输出
═══════════════════════════════════════════════════════════════
  Op15: M0 + M7 → output  [fused_add_3]   结果: 235

  输出: 235.0
```

### 4.2 内存布局

| 槽位 | 偏移 | 大小 | 生命周期 | 复用次数 |
|------|------|------|----------|---------|
| M0 | ws[0] | 4B | L1写→L3复用→L8复用 | 3次 |
| M1 | ws[8] | 4B | L1写→L3复用 | 2次 |
| M2 | ws[16] | 4B | L1写→L5复用 | 2次 |
| M3 | ws[24] | 4B | L1写→L5复用 | 2次 |
| M4 | ws[32] | 4B | L2写→L7复用 | 2次 |
| M5 | ws[40] | 4B | L2写→L7复用 | 2次 |
| M6 | ws[48] | 4B | L4写 | 1次 |
| M7 | ws[56] | 4B | L6写 | 1次 |

**Workspace 总大小**: 64 bytes

### 4.3 内存生命周期详表

| 槽 | L1 | L2 | L3 | L4 | L5 | L6 | L7 | L8 | L9 |
|----|----|----|----|----|----|----|----|----|-----|
| M0 | 写11 | 读 | 🔴22 | 读 | - | - | - | 🔴139 | 读→out |
| M1 | 写13 | 读 | 🔴22 | 读 | - | - | - | - | - |
| M2 | 写15 | 读 | - | - | 🔴47 | 读 | - | - | - |
| M3 | 写11 | 读 | - | - | 🔴49 | 读 | - | - | - |
| M4 | - | 写24 | 读 | - | - | - | 🔴94 | 读 | - |
| M5 | - | 写26 | 读 | - | - | - | 🔴45 | 读 | - |
| M6 | - | - | - | 写44 | 读 | - | 读 | - | - |
| M7 | - | - | - | - | - | 写96 | 读 | - | 读→out |

🔴 = 内存复用点

### 4.4 常量表

| 偏移 | 值 | 使用算子 |
|------|-----|----------|
| 0 | 5.0 | `fused_add_2` (+5) |
| 16 | 4.0 | `fused_subtract_1` (-4) |
| 32 | 3.0 | `fused_add_1` (+3) |
| 48 | 2.0 | `fused_subtract` (-2) |
| 64 | 1.0 | `fused_add` (+1) |

**常量 Workspace 总大小**: 68 bytes

---

## 5. 函数列表

### 5.1 `src/main.c` (测试入口)

| 函数 | 说明 |
|------|------|
| `main()` | 程序入口，准备数据并调用推理，验证结果为 235.0 |

### 5.2 `src/default_lib0.c` (Workspace)

| 函数/数据 | 说明 |
|----------|------|
| `global_const_workspace` | 静态分配的常量区 (68 bytes, 5个常量) |
| `global_workspace` | 静态分配的可变 workspace (64 bytes, 8槽) |
| `tvmgen_default_run()` | 运行入口，调用 `__tvm_main__` |

### 5.3 `src/default_lib1.c` (Runtime 初始化)

| 函数 | 说明 |
|------|------|
| `init_op_execs()` | 初始化 16 个算子的执行表 |
| `tvmgen_default___tvm_main__()` | TVM 主入口，初始化并运行调度引擎 |

### 5.4 `src/tvmrt.c` (Runtime 核心)

#### 日志系统
| 函数 | 说明 |
|------|------|
| `tvmrt_log_set_callback()` | 设置日志回调 |
| `tvmrt_log_push()` | 压入日志记录 |
| `tvmrt_log_pop()` | 弹出日志记录 |
| `tvmrt_log_clear()` | 清空日志 |
| `tvmrt_log_count()` | 获取日志数量 |

#### 语义转换层
| 函数 | 说明 |
|------|------|
| `tvmrt_semantic_init()` | 初始化运行时上下文 |
| `tvmrt_semantic_resolve_sid()` | 解析 Storage ID 到指针 |

#### 调度引擎
| 函数 | 说明 |
|------|------|
| `tvmrt_engine_init()` | 初始化调度引擎（创建线程池） |
| `tvmrt_engine_shutdown()` | 关闭调度引擎 |
| `tvmrt_engine_run()` | 执行 BSP 调度（多线程） |
| `tvmrt_engine_run_single()` | 单线程执行（当前默认使用） |
| `load_next_layer()` | 辅助函数：加载下一层任务 |
| `worker_func()` | Worker 线程函数 |

### 5.5 `src/tvmrt_port_posix.c` (OS 适配)

| 函数 | 说明 |
|------|------|
| `tvmrt_mutex_init/lock/unlock/destroy()` | 互斥锁操作 |
| `tvmrt_cond_init/wait/signal/broadcast/destroy()` | 条件变量操作 |
| `tvmrt_thread_create/join()` | 线程操作 |
| `tvmrt_barrier_init/reset/arrive/sync/destroy()` | 屏障操作 |

### 5.6 `src/model_data.c` (模型描述)

| 函数 | 说明 |
|------|------|
| `model_get_descriptor()` | 获取模型描述符 |
| `model_get_schedule()` | 获取静态调度表 (9层) |
| `model_fill_args()` | 填充 16 个算子的参数 |
| `model_get_op_args()` | 获取指定算子的参数指针 |

### 5.7 `src/ops.c` (算子)

#### 基础算子（6 种）

| 函数 | 操作 |
|------|------|
| `tvmgen_default_fused_add()` | out = p0 + 1.0 |
| `tvmgen_default_fused_add_1()` | out = p0 + 3.0 |
| `tvmgen_default_fused_add_2()` | out = p0 + 5.0 |
| `tvmgen_default_fused_add_3()` | out = p0 + p1 |
| `tvmgen_default_fused_subtract()` | out = p0 - 2.0 |
| `tvmgen_default_fused_subtract_1()` | out = p0 - 4.0 |

#### 激活函数（4 种）

| 函数 | 操作 |
|------|------|
| `tvmgen_default_relu()` | ReLU: max(0, x) |
| `tvmgen_default_sigmoid()` | Sigmoid: 1 / (1 + exp(-x)) |
| `tvmgen_default_tanh_op()` | Tanh: tanh(x) |
| `tvmgen_default_relu6()` | ReLU6: min(max(0, x), 6) |

#### 基础运算（3 种）

| 函数 | 操作 |
|------|------|
| `tvmgen_default_multiply()` | out = p0 * p1 |
| `tvmgen_default_maximum()` | out = max(p0, p1) |
| `tvmgen_default_minimum()` | out = min(p0, p1) |

#### 常量运算（2 种）

| 函数 | 操作 |
|------|------|
| `tvmgen_default_mul_2()` | out = p0 * 2.0 |
| `tvmgen_default_mul_half()` | out = p0 * 0.5 |

#### 包装函数

所有算子都有对应的 `wrapped_*()` 包装函数，适配统一签名。

---

## 6. 运行流程

### 6.1 调用顺序

```
main()
  ├─ 分配: input_buffer[1] = 10.0, output_buffer[1]
  └─ 调用: tvmgen_default_run(&inputs, &outputs)

tvmgen_default_run() [default_lib0.c]
  ├─ 使用: global_const_workspace (68B), global_workspace (64B)
  └─ 调用: tvmgen_default___tvm_main__(input, output, const_ws, ws)

tvmgen_default___tvm_main__() [default_lib1.c]
  ├─ 调用: tvmrt_engine_init()
  │
  ├─ 调用: init_op_execs()
  │   ├─ model_fill_args()      ← 填充 16 个算子参数
  │   └─ model_get_op_args()    ← 获取参数指针
  │
  ├─ 构造: tvmrt_context_t ctx (16 个算子)
  └─ 调用: tvmrt_engine_run_single(&ctx, schedule)

tvmrt_engine_run_single() [tvmrt.c]
  └─ 逐层执行:
       Layer 1 → Layer 2 → ... → Layer 9
       共 16 个算子
```

### 6.2 BSP 调度层级

```
┌─────────────────────────────────────────────────────────────┐
│                    Layer 1 (队列: [0, 1, 2, 3])             │
│   ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐       │
│   │  Op 0   │  │  Op 1   │  │  Op 2   │  │  Op 3   │       │
│   │  +1→M0  │  │  +3→M1  │  │  +5→M2  │  │  +1→M3  │       │
│   └─────────┘  └─────────┘  └─────────┘  └─────────┘       │
├─────────────────────────────────────────────────────────────┤
│                    ═══ BARRIER ═══                          │
├─────────────────────────────────────────────────────────────┤
│                    Layer 2 (队列: [4, 5])                   │
│   ┌───────────────┐    ┌───────────────┐                   │
│   │     Op 4      │    │     Op 5      │                   │
│   │ M0+M1→M4=24   │    │ M2+M3→M5=26   │                   │
│   └───────────────┘    └───────────────┘                   │
├─────────────────────────────────────────────────────────────┤
│                    ═══ BARRIER ═══                          │
├─────────────────────────────────────────────────────────────┤
│                    Layer 3 (队列: [6, 7])                   │
│   ┌───────────────┐    ┌───────────────┐                   │
│   │     Op 6      │    │     Op 7      │                   │
│   │ M4-2→M0🔴=22  │    │ M5-4→M1🔴=22  │                   │
│   └───────────────┘    └───────────────┘                   │
├─────────────────────────────────────────────────────────────┤
│                    ... 继续到 Layer 9 ...                   │
└─────────────────────────────────────────────────────────────┘
```

### 6.3 数据流

```
input (10.0)
    │
    ├──▶ Op0: +1 ──▶ M0 (11) ──┬──▶ Op4: +M1 ──▶ M4 (24) ──▶ Op6: -2 ──▶ M0🔴 (22)
    │                          │                                              │
    ├──▶ Op1: +3 ──▶ M1 (13) ──┘                                              │
    │                                                                         │
    ├──▶ Op2: +5 ──▶ M2 (15) ──┬──▶ Op5: +M3 ──▶ M5 (26) ──▶ Op7: -4 ──▶ M1🔴 (22)
    │                          │                                              │
    └──▶ Op3: +1 ──▶ M3 (11) ──┘                                              │
                                                                              │
    M0 (22) + M1 (22) ──▶ Op8 ──▶ M6 (44) ──┬──▶ Op9:  +3 ──▶ M2🔴 (47)      │
                                             │                                │
                                             └──▶ Op10: +5 ──▶ M3🔴 (49)      │
                                                                              │
    M2 (47) + M3 (49) ──▶ Op11 ──▶ M7 (96) ──┬──▶ Op12: -2 ──▶ M4🔴 (94)     │
                                              │                               │
    M6 (44) ────────────────────────────────┬┴──▶ Op13: +1 ──▶ M5🔴 (45)     │
                                            │                                 │
    M4 (94) + M5 (45) ──▶ Op14 ──▶ M0🔴 (139)                                │
                                  │                                          │
    M0 (139) + M7 (96) ──▶ Op15 ──▶ output (235)
```

---

## 7. 核心类型定义

### 7.1 数据结构 (定义在 `tvmrt.h`)

| 结构体 | 说明 |
|--------|------|
| `tvmrt_layer_queue_t` | 层级任务队列（存储当前层待执行算子） |
| `tvmrt_context_t` | 运行时上下文（workspace、算子执行表） |
| `tvmrt_op_exec_t` | 可执行算子条目（函数指针 + 参数） |
| `tvmrt_schedule_desc_t` | 静态调度表（9层数组） |
| `tvmrt_schedule_layer_t` | 单个调度层（算子 ID 数组） |
| `tvmrt_op_desc_t` | 算子描述（名称、后端、输入输出 SID） |
| `tvmrt_tensor_map_entry_t` | Tensor 内存映射项 |

### 7.2 全局静态参数

#### Workspace (default_lib0.c)

```c
static struct global_const_workspace {
    float fused_constant_4_let[1];  // offset 0:  5.0
    float fused_constant_3_let[1];  // offset 16: 4.0
    float fused_constant_2_let[1];  // offset 32: 3.0
    float fused_constant_1_let[1];  // offset 48: 2.0
    float fused_constant_let[1];    // offset 64: 1.0
} global_const_workspace = {...};   // 68 bytes

static uint8_t global_workspace[64];  // 8个内存槽
```

#### 模型描述参数 (model_data.c)

| 参数 | 类型 | 说明 |
|------|------|------|
| `g_tensor_map[]` | `tvmrt_tensor_map_entry_t[]` | 张量内存映射表（12个 SID） |
| `g_op_descs[]` | `tvmrt_op_desc_t[]` | 算子描述表（16个节点） |
| `g_cpu_func_table[]` | `tvmrt_op_func_t[]` | CPU 函数指针表（6种算子） |
| `g_schedule_layers[]` | `tvmrt_schedule_layer_t[]` | 静态调度表（9层） |
| `g_single_args[16]` | `FusedAddArgs[]` | 单输入算子参数 |
| `g_dual_args[16]` | `FusedAdd3Args[]` | 双输入算子参数 |

#### 引擎状态 (tvmrt.c)

| 参数 | 类型 | 说明 |
|------|------|------|
| `g_engine` | `engine_state_t` | 全局引擎状态 |
| `g_engine.task_queue` | `tvmrt_layer_queue_t` | 当前层任务队列 |
| `g_engine.workers[4]` | `tvmrt_thread_t[]` | Worker 线程数组 |
| `g_engine.layer_barrier` | `tvmrt_barrier_t` | 层间同步屏障 |

---

## 8. 编译与运行

### 8.1 快速开始

```bash
# 编译
make all

# 运行主程序（回归测试）
make run

# 运行单元测试
make test

# 清理
make clean
```

### 8.2 预期输出

#### 默认模式（日志启用）

```
========================================
  TVM Runtime: 16算子 / 9层 / 8内存槽
========================================
输入值: 10.0
预期输出: 235.0

执行中...
=== Layer 1 (4 ops) ===
[DEBUG][W-1] fused_add: p0=10.00 → output@0x...
[INFO][W-1] fused_add → result=11.00
[DEBUG][W-1] fused_add_1: p0=10.00 → output@0x...
[INFO][W-1] fused_add_1 → result=13.00
...
=== Layer 9 (1 op) ===
[DEBUG][W-1] fused_add_3: p0=139.00, p1=96.00 → output@0x...
[INFO][W-1] fused_add_3 → result=235.00

--- 结果 ---
执行状态: 成功
实际输出: 235.0

✅ 测试通过! 结果正确
========================================
```

#### 零开销模式（日志禁用）

```bash
make LOG_ENABLE=0
```

```
========================================
  TVM Runtime: 16算子 / 9层 / 8内存槽
========================================
输入值: 10.0
预期输出: 235.0

执行中...
=== Layer 1 (4 ops) ===
=== Layer 2 (2 ops) ===
...
=== Layer 9 (1 op) ===

--- 结果 ---
执行状态: 成功
实际输出: 235.0

✅ 测试通过! 结果正确
========================================
```

### 8.3 编译选项

```makefile
# Makefile 配置
CC = clang
CFLAGS = -Isrc -Iinclude -Wno-everything -g -O2

# 日志开关 (1=启用, 0=零开销禁用)
LOG_ENABLE ?= 1
CFLAGS += -DTVMRT_LOG_ENABLE=$(LOG_ENABLE)
```

```c
// tvmrt.h 中配置
#define TVMRT_NUM_WORKERS 4      // Worker 线程数 (0=单线程)
#define TVMRT_LOG_ENABLE 1       // 日志开关 (通过 Makefile 设置)
#define TVMRT_MAX_OPS 64         // 最大算子数
#define TVMRT_MAX_LAYERS 32      // 最大层数
```

**日志开关使用**:

```bash
# 启用日志（默认）
make
make LOG_ENABLE=1

# 禁用日志（零运行时开销）
make LOG_ENABLE=0
```

---

## 9. 架构优势

### 9.1 职责清晰

| 组件 | 职责 | 文件 |
|------|------|------|
| **入口层** | 测试与外部接口 | `main.c` |
| **Workspace 层** | 内存分配与生命周期 | `default_lib0.c` |
| **初始化层** | Runtime 对接与初始化 | `default_lib1.c` |
| **Runtime 层** | 调度引擎与工具函数 | `tvmrt.h/c` |
| **OS 适配层** | 平台抽象 | `tvmrt_port_posix.c` |
| **模型层** | 编译器生成的描述 | `model_data.c` |
| **算子层** | 编译器生成的实现 | `ops.c` |

### 9.2 可移植性

- ✅ **单一 Runtime API**：只需 `#include "tvmrt.h"`
- ✅ **OS 适配分离**：替换 `tvmrt_port_*.c` 即可移植
- ✅ **职责清晰**：lib0 负责内存，lib1 负责初始化
- ✅ **零外部依赖**：除 pthread 外无其他依赖
- ✅ **静态内存**：零动态分配，适合嵌入式

### 9.3 极限内存复用

本模型验证了复杂的内存复用场景：
- 8 个内存槽支撑 16 个算子
- 每个槽被复用 1-3 次
- 仅 64 bytes workspace

---

## 10. 扩展与定制

### 10.1 替换 OS 适配层

创建 `tvmrt_port_rtos.c` 实现 RTOS 线程接口：

```c
// 实现以下函数即可
int tvmrt_mutex_init(tvmrt_mutex_t* m);
int tvmrt_cond_wait(tvmrt_cond_t* c, tvmrt_mutex_t* m);
// ... 其他接口
```

### 10.2 启用多线程

修改 `default_lib1.c`：
```c
// 将
int32_t ret = tvmrt_engine_run_single(&ctx, schedule);
// 改为
int32_t ret = tvmrt_engine_run(&ctx, schedule);
```

### 10.3 更换模型

1. 修改 `model_data.c`（描述表和调度表）
2. 如需新算子，修改 `ops.c`
3. 更新 `default_lib0.c` 中的 workspace 大小

---

## 11. 文件依赖关系

```
main.c
  └─▶ tvmgen_default.h
  └─▶ default_lib0.c
        └─▶ tvmgen_default.h
        └─▶ default_lib1.c
              └─▶ tvmrt.h
              └─▶ model_data.c
                    └─▶ tvmrt.h
                    └─▶ ops.c
                          └─▶ tvmrt.h
                          └─▶ tvmrt.c
                                └─▶ tvmrt.h
                                └─▶ tvmrt_port_posix.c
                                      └─▶ tvmrt.h
```

---

## 12. 常见问题

**Q: 如何调整 Worker 线程数？**
A: 修改 `tvmrt.h` 中的 `TVMRT_NUM_WORKERS`

**Q: 如何启用/禁用日志？**
A: 使用 `make LOG_ENABLE=1` 启用日志，`make LOG_ENABLE=0` 禁用日志（零开销）

**Q: 日志输出格式是什么意思？**
A: 日志包含两种类型：
   - `[DEBUG] 算子名: p0=x → output@地址` - 执行前的输入参数
   - `[INFO] 算子名 → result=x` - 执行后的计算结果

**Q: 为什么使用单线程模式？**
A: 当前默认使用 `tvmrt_engine_run_single` 以确保稳定性

**Q: lib0 和 lib1 的区别？**
A:
- `lib0`: 负责 **内存分配**（workspace）和 **外部接口**
- `lib1`: 负责 **Runtime 初始化**和 **调度执行**

**Q: 内存复用是如何实现的？**
A: 通过在 `model_data.c` 中将不同 SID 映射到相同的 workspace 偏移

**Q: 如何验证内存复用是否正确？**
A: 启用日志后，观察相同 `output@` 地址被多次写入时的 `result` 值变化

**Q: 如何运行单元测试？**
A: 使用 `make test` 运行 14 项新算子的单元测试

**Q: 单元测试覆盖了哪些算子？**
A: 激活函数（ReLU, Sigmoid, Tanh, ReLU6）、基础运算（Multiply, Maximum, Minimum）、常量运算（Mul2, MulHalf）

---

```
总计: ~2300 行代码（含注释）

入口层:
- main.c:           103 行
- default_lib0.c:    64 行
- default_lib1.c:    88 行

Runtime 层:
- tvmrt.h:          464 行
- tvmrt.c:          401 行
- tvmrt_port_posix.c: 135 行

模型层:
- model_data.c:     447 行
- ops.c:            300 行
- test_new_ops.c:   149 行

文档:
- README.md:        700+ 行
- future.md:        230+ 行
- docs/updates/:    300+ 行
```

---

## 14. 许可与引用

本项目为 TVM Runtime 的精简嵌入式实现示例。

**参考资料**:
- TVM 官方文档: https://tvm.apache.org/
- BSP 模型: Bulk Synchronous Parallel

---

**最后更新**: 2026-01-15
**模型规格**: 16 算子 / 9 层 / 8 内存槽
**算子总数**: 15 种
**预期结果**: input=10.0 → output=235.0
**单元测试**: 14 项全部通过
